...
import { Send, CornerDownLeft, Sparkles, MessageSquareCode, ShieldCheck } from "lucide-react";
import { ChatMessage } from "../../data/scenarios";
    ...
    <div className="glass-panel rounded-xl flex flex-col h-[500px] lg:h-full relative overflow-hidden">
      ...
      <div className="px-4 py-3.5 border-b border-zinc-800/80 flex items-center justify-between bg-zinc-950/20">
        ...
        <div className="flex items-center gap-1">
          <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-mono font-medium">
            AI Agent Driving
          </span>
        </div>
      </div>
      ...
      >
        ...
        {currentStepIndex === 0 && chatMessages.length === 1 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
      ...
      <div className="p-4 border-t border-zinc-800/80 bg-zinc-950/40 relative">
        <div className="relative flex items-center rounded-xl border border-zinc-800 bg-zinc-900/40 px-3 py-2.5">
          <input
            disabled
            placeholder="AI is executing workflow steps. Input paused..."
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              disabled
              className="rounded-lg p-1.5 bg-zinc-800 text-zinc-600 disabled:opacity-50 cursor-not-allowed"
            >
              <Send size={12} />
            </button>
          </div>
